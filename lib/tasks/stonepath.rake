namespace :stonepath do
  ...
  task :diagrams => :environment do
    FileUtils.mkdir_p "#{Rails.root}/doc/stonepath/dot"
    FileUtils.mkdir_p "#{Rails.root}/doc/stonepath/png"
    workitems.each do |workitem|
      dot_file = File.join(Rails.root, 'doc', 'stonepath', 'dot', "#{workitem.to_s.tableize.singularize}.dot")
      png_file = File.join(Rails.root, 'doc', 'stonepath', 'png', "#{workitem.to_s.tableize.singularize}.png")
      File.open(dot_file, 'w') {|f| f.write(workitem.to_dot) }
      `dot -Tpng "#{dot_file}" > "#{png_file}"`
    end
    
    tasks.each do |task|
      dot_file = File.join(Rails.root, 'doc', 'stonepath', 'dot', "#{task.to_s.tableize.singularize}.dot")
      png_file = File.join(Rails.root, 'doc', 'stonepath', 'png', "#{task.to_s.tableize.singularize}.png")
...
private
  ...
  def self.implementing_models(a_module)
    tasks = Array.new
    Dir[File.join(Rails.root, "app/models/**/*.rb")].each do |file_path|
